...
private:
    PinOutputPWM_t<LED_TOP_VALUE, LED_INVERTED_PWM, omPushPull> IChnl;
...
public:
    LedSmooth_t(const PinOutputPWM_t<LED_TOP_VALUE, LED_INVERTED_PWM, omPushPull> AChnl) :
        BaseSequencer_t(), IChnl(AChnl), ICurrentBrightness(0) {}
    void Init(uint32_t FreqHz = 1000000) {
        IChnl.Init();
        IChnl.SetFrequencyHz(FreqHz);
        SetBrightness(0);
    }
